...
class MyDataset(Data.Dataset):
    ...
    def __init__(self, pickle_name):
        self.pair = ()
        with open(pickle_name,'rb') as func:
            self.data = pickle.load(func)
        for i in range(len(self.data)):
            stack = self.data[i]
            for j in range(2*radius+1):
                self.pair = self.pair + ((stack[j,:,:],stack[radius,:,:]),)
...
train_loader = Data.DataLoader(dataset=MyDataset('E:\\vm_train.pickle'),
    ...
    for step, [m_img,f_img] in enumerate(train_loader):
        ...
        if step == 0 and epoch != 0:
            with torch.no_grad():
                ...
                im_fix = f_img[0,0,:,:].numpy()
                im_mov = m_img[0,0,:,:].numpy()
                im_reg = warp[0,0,:,:].detach().cpu().numpy()
                
                plt.figure(figsize=(18,6))
                plt.axis('off')
                plt.imshow(np.concatenate((im_fix,im_reg,im_mov),axis=1),cmap='gray')
                plt.show()
